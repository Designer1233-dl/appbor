...
HOSTED_DATA_DB_PATH = Path("/app/data/bor.db")
...
def resolve_port() -> int:
    raw = os.getenv("BOR_PORT") or os.getenv("PORT")
    if raw is None:
        return 8080
    try:
...
def resolve_db_path() -> Path:
    value = os.getenv("BOR_DB_PATH")
    if value and value.strip():
        return Path(value.strip())
    if HOSTED_DATA_DB_PATH.parent.exists():
        return HOSTED_DATA_DB_PATH
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def serve_static(self, request_path: str) -> None:
        ...
        if not str(file_path).startswith(public_root):
            self.send_json({"error": "File not found"}, status=HTTPStatus.NOT_FOUND)
            return
        if not file_path.exists() or file_path.is_dir():
            # Telegram Web Apps and some hosts may open a custom path.
            # For non-asset routes we fall back to the main entry page.
            if Path(relative).suffix:
                self.send_json({"error": "File not found"}, status=HTTPStatus.NOT_FOUND)
                return
            file_path = (PUBLIC_DIR / "index.html").resolve()
